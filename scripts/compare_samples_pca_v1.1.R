...
subsets <- getNodes(gs_list[[1]],order="bfs",path="auto")[c(9)]
...
############################
set.seed(6)
pred <- Rtsne(as.matrix(my.data[-c(1,2)]),theta=0.75)
pred <- as.data.frame(pred)

## 2D GGPLOT OF PC1 AND PC2
###########################
g <- ggplot( pred, aes(Y.1,Y.2)) +
  geom_point(size=1)+
  theme_bw()
g
# g <- ggplot( pred, aes(PC1, PC2,color=sample,shape=subset)) +
#   geom_point(size=1)+
#   theme_bw()+
#   labs(colour="Sample",shape="T Cell Subset")+
