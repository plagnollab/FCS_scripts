...
subsets <- getNodes(gs,order="bfs",path="auto")[c(11:15)]
...
pred <- Rtsne(as.matrix(my.sampled.data[,-c(1)]),theta=0.5,check_duplicates=FALSE)
pred <- as.data.frame(pred)

## 2D GGPLOT OF PC1 AND PC2
###########################
pred <- cbind(my.sampled.data[,c(1)],pred)
colnames(pred) <- c("subset",colnames(pred)[-1])
g <- ggplot( pred, aes(x=Y.1,y=Y.2,colour=subset)) +
  geom_point(aes(colour=subset),size=2)+
  #theme_bw()+
  labs(colour="Subset")+
  xlab("")+
  ylab("")+
...
print(g)
ggsave(file=paste("/Users/niclasthomas/Dropbox/pid/tregfigures/",filename,".png",sep=""),g,scale=1,width=10,height=10,dpi=300)
